...
class PrehledovyListParser(IsirParser):
    """Parser pro čtení formulářů typu Přehledový list.
    """
    ...
    def _prehledZajistenych(self):
        txt = self.reTextBetween(
            self.txt, "^[\s]*B. Přehled přihlášených zajištěných pohledávek", "^[\s]*Komentář:")
    ...
    def _prehledNezajistenych(self):
        txt = self.reTextBetween(
            self.txt, "^[\s]*C. Přehled přihlášených nezajištěných pohledávek", "^[\s]*Komentář:")
        pohledavky, celkem = self._prehledPohledavek(txt)
