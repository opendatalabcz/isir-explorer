...
class ZpravaSplneniOddluzeniParser(IsirParser):
    ...
    def _prubehRizeni(self):
        txt = self.reTextBetween(
            self.txt, "^[\s]*A\. Průběh insolvenčního řízení", "^[\s]*B\. Výsledek insolvenčního řízení")

        self.model.Prubeh_rizeni.Zahajeno = self.reLineTextAfter(
            txt, "^[\s]*Insolvenční řízení zahájeno dne")
        self.model.Prubeh_rizeni.Zjisteni_upadku = self.reLineTextAfter(
            txt, "^[\s]*Úpadek dlužníka zjištěn usnesením ze dne")
        self.model.Prubeh_rizeni.Oddluzeni_povoleno = self.reLineTextAfter(
            txt, "^[\s]*Oddlužení povoleno usnesením ze dne")
        self.model.Prubeh_rizeni.Oddluzeni_schvaleno = self.reLineTextAfter(
            txt, "^[\s]*Oddlužení schváleno usnesením ze dne")

    def _vysledekRizeni(self):
        txt = self.reTextBetween(
            self.txt, "^[\s]*B\. Výsledek insolvenčního řízení", "^[\s]*C\. Pohledávky za majetkovou podstatou")

        self.model.Vysledek_rizeni.Posledni_splatka = self.reLineTextAfter(
            txt, "^[\s]*Poslední splátka dle splátkového kalendáře")
        self.model.Vysledek_rizeni.Preplatek = self.priceValue(
            self.reLineTextAfter(txt, "^[\s]*Přeplatek na splátkách ve výši"))
        ...
        for line in lines:
            if self.reMatch(line, '^[\s]*Předpokládaná míra uspokojení nezajištěných'):
                ...
                row = self.reTextAfter(
                    line, '^[\s]*Nezajištění věřitelé uspokojeni \(výše / míra\)')
                cols = re.compile("[\s]{2,}").split(row.strip())
                if len(cols) == 2:
                    self.model.Vysledek_rizeni.Uspokojeni_nezaj_vyse = self.priceValue(
                        cols[0])
                    self.model.Vysledek_rizeni.Uspokojeni_nezaj_mira = self.priceValue(
                        cols[1])
                continue
            elif self.reMatch(line, '^[\s]*Zajištění věřitelé uspokojeni \(výše / míra\)'):
                row = self.reTextAfter(
                    line, '^[\s]*Zajištění věřitelé uspokojeni \(výše / míra\)')
                cols = re.compile("[\s]{2,}").split(row.strip())
                if len(cols) == 2:
                    self.model.Vysledek_rizeni.Uspokojeni_zaj_vyse = self.priceValue(
                        cols[0])
                    self.model.Vysledek_rizeni.Uspokojeni_zaj_mira = self.priceValue(
                        cols[1])
                continue
            ...
            if nextLineState == USPOKOJENI_NEZAJ:
                ...
                if len(cols) == 2:
                    self.model.Vysledek_rizeni.Predpoklad_uspokojeni_nezaj_vyse = self.priceValue(
                        cols[0])
                    self.model.Vysledek_rizeni.Predpoklad_uspokojeni_nezaj_mira = self.priceValue(
                        cols[1])
                    nextLineState = 0
    ...
    def _vyuctovaniOdmenyIS(self):
        ...
        for line in lines:
            if self.reMatch(line, '^[\s]*Celková odměna insolvenčního'):
                ...
                nextLineState = 0
                row = self.reTextAfter(
                    line, '^[\s]*výtěžku zpeněžení zajištěného')
                cols = re.compile("[\s]{2,}").split(row.strip())
                if len(cols) == 3:
                    # prvni sloupec je cena bez dph pokud je platce dph a je mozne jej ingorovat
                    cols = cols[1:]
                if len(cols) == 2:
                    self.model.Odmena_spravce.Vytezek_zpenezeni_zaji = self.priceValue(
                        cols[0])
                    self.model.Odmena_spravce.Vytezek_zpenezeni_zaji_uhrazeno = self.priceValue(
                        cols[1])
                continue
            elif self.reMatch(line, '^[\s]*výtěžku zpeněžení určeného k'):
                nextLineState = 0
                row = self.reTextAfter(
                    line, '^[\s]*výtěžku zpeněžení určeného k')
                cols = re.compile("[\s]{2,}").split(row.strip())
                if len(cols) == 3:
                    # prvni sloupec je cena bez dph pokud je platce dph a je mozne jej ingorovat
                    cols = cols[1:]
                if len(cols) == 2:
                    self.model.Odmena_spravce.Vytezek_zpenezeni_rozdeleni = self.priceValue(
                        cols[0])
                    self.model.Odmena_spravce.Vytezek_zpenezeni_rozdeleni_uhrazeno = self.priceValue(
                        cols[1])
                break  # Konec tabulky
            ...
            if nextLineState == CELKOVA_ODMENA:
                ...
                if len(cols) == 3:
                    ...
                    cols = cols[1:]
                if len(cols) == 2:
                    self.model.Odmena_spravce.Celkova_odmena = self.priceValue(
                        cols[0])
                    self.model.Odmena_spravce.Celkova_odmena_uhrazeno = self.priceValue(
                        cols[1])
                    nextLineState = 0
                ...
                if len(cols) == 3:
                    ...
                    cols = cols[1:]
                if len(cols) == 2:
                    self.model.Odmena_spravce.Hotove_vydaje = self.priceValue(
                        cols[0])
                    self.model.Odmena_spravce.Hotove_vydaje_uhrazeno = self.priceValue(
                        cols[1])
                    nextLineState = 0
        ...
        self.model.Odmena_spravce.Zprava_spravce = self.textBlock(
            self.reTextAfter(txt, "^[\s]*Komentář:[\s]*$", True))
